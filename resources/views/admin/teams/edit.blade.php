...
<div class="mb-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Edit profil {{ $team->name }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="px-6 py-4 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="bio" id="bio" rows="4"
                                  ...
                                  placeholder="Ceritakan tentang pengalaman dan keahlian anggota tim...">{{ old('bio', $team->bio) }}</textarea>
                        @error('bio')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">Format: JPG, PNG, WEBP. Maksimal 2MB. Kosongkan jika tidak ingin mengubah.</p>
                        @error('photo')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="url" name="instagram" id="instagram" 
                                   value="{{ old('instagram', $team->social_links['instagram'] ?? '') }}"
                        ...
                        <div>
                            ...
                            <input type="url" name="facebook" id="facebook" 
                                   value="{{ old('facebook', $team->social_links['facebook'] ?? '') }}"
        ...
        <div class="lg:col-span-1">
            ...
            <div class="bg-white shadow-sm border border-gray-200 rounded-lg mb-6">
                ...
                <div class="px-6 py-4 space-y-3">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="text-gray-900">{{ $team->created_at->format('d M Y H:i') }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Diupdate:</span>
                        <span class="text-gray-900">{{ $team->updated_at->format('d M Y H:i') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white shadow-sm border border-gray-200 rounded-lg">
                ...
                <div class="p-6">
                    <!-- Photo Preview -->
                    <div id="photo-preview-container" class="mb-4 {{ $team->photo ? '' : 'hidden' }}">
                        <img id="photo-preview" 
                             src="{{ $team->photo ? Storage::url($team->photo) : '' }}"
                             class="w-20 h-20 object-cover rounded-full mx-auto">
                    </div>
                    
                    <!-- Live Preview -->
                    <div class="text-center">
                        <div class="w-20 h-20 bg-gray-200 rounded-full mx-auto mb-3 flex items-center justify-center {{ $team->photo ? 'hidden' : '' }}" id="default-avatar">
                            <i class="fas fa-user text-gray-400 text-2xl"></i>
                        </div>
                        <h4 id="preview-name" class="font-semibold text-gray-900 mb-1">{{ $team->name }}</h4>
                        <p id="preview-position" class="text-sm text-gray-600 mb-3">{{ $team->position }}</p>
                        <div id="preview-contact" class="text-xs text-gray-500 space-y-1 mb-3">
                            <div id="preview-email" class="{{ $team->email ? '' : 'hidden' }}">
                                <i class="fas fa-envelope mr-1"></i>
                                <span>{{ $team->email }}</span>
                            </div>
                            <div id="preview-phone" class="{{ $team->phone ? '' : 'hidden' }}">
                                <i class="fas fa-phone mr-1"></i>
                                <span>{{ $team->phone }}</span>
                            </div>
                        </div>                        <div id="preview-social" class="flex justify-center space-x-2 mb-3">
                            @if($team->social_links && isset($team->social_links['linkedin']) && $team->social_links['linkedin'])
                                <a href="#" class="text-blue-600">
                                    <i class="fab fa-linkedin"></i>
                                </a>
                            @endif
                            @if($team->social_links && isset($team->social_links['twitter']) && $team->social_links['twitter'])
                                <a href="#" class="text-blue-400">
                                    <i class="fab fa-twitter"></i>
                                </a>
                            @endif
                            @if($team->social_links && isset($team->social_links['instagram']) && $team->social_links['instagram'])
                                <a href="#" class="text-pink-600">
                                    <i class="fab fa-instagram"></i>
                                </a>
                            @endif
                            @if($team->social_links && isset($team->social_links['facebook']) && $team->social_links['facebook'])
                                <a href="#" class="text-blue-700">
                                    <i class="fab fa-facebook"></i>
                                </a>
                            @endif
                        </div>
                        <div id="preview-bio" class="text-xs text-gray-600 italic {{ $team->bio ? '' : 'hidden' }}">
                            {{ $team->bio }}
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateSocialLinks() {
        const existingIcons = previewSocial.querySelectorAll('a');
        ...
        if (facebookInput.value) {
            ...
            previewSocial.appendChild(facebookIcon);
        }
    }
    ...
    facebookInput.addEventListener('input', updateSocialLinks);
});
</script>
@endpush
@endsection
