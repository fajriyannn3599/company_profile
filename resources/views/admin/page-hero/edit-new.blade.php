...
<div class="mb-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Page Hero</h1>
            <p class="mt-1 text-sm text-gray-600">Edit hero section untuk halaman {{ ucfirst($pageHero->page_identifier) }}</p>
        </div>
        <div class="mt-4 sm:mt-0 flex space-x-3">
            <a href="{{ route('admin.page-hero.show', $pageHero) }}" 
...
<form action="{{ route('admin.page-hero.update', $pageHero) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white shadow-sm border border-gray-200 rounded-lg">
                <div class="px-6 py-4 border-b border-gray-200">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Edit informasi dasar hero section</p>
                </div>
                <div class="px-6 py-4 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="page_identifier" id="page_identifier" 
                                ...
                                @foreach($pages as $key => $value)
                                    <option value="{{ $key }}" {{ old('page_identifier', $pageHero->page_identifier) == $key ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="height" id="height" 
                                ...
                                <option value="small" {{ old('height', $pageHero->height) == 'small' ? 'selected' : '' }}>Small (40% viewport)</option>
                                <option value="medium" {{ old('height', $pageHero->height) == 'medium' ? 'selected' : '' }}>Medium (60% viewport)</option>
                                <option value="large" {{ old('height', $pageHero->height) == 'large' ? 'selected' : '' }}>Large (80% viewport)</option>
                                <option value="fullscreen" {{ old('height', $pageHero->height) == 'fullscreen' ? 'selected' : '' }}>Fullscreen</option>
                            </select>
                            @error('height')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea name="subtitle" id="subtitle" rows="3"
                                  ...
                                  placeholder="Masukkan subjudul (opsional)">{{ old('subtitle', $pageHero->subtitle) }}</textarea>
                        @error('subtitle')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            ...
                            @if($pageHero->background_image)
                                <br>Current: <a href="{{ Storage::url($pageHero->background_image) }}" target="_blank" class="text-blue-600 hover:text-blue-800">Lihat gambar saat ini</a>
                            @endif
                        </p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="color" name="background_overlay_color" id="background_overlay_color" 
                                   value="{{ old('background_overlay_color', $pageHero->background_overlay_color) }}"
                        ...
                        <div>
                            ...
                            <input type="range" name="background_overlay_opacity" id="background_overlay_opacity" 
                                   min="0" max="100" value="{{ old('background_overlay_opacity', $pageHero->background_overlay_opacity) }}"
                                   class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer slider"
                                   oninput="document.getElementById('opacity-value').textContent = this.value">
                            <div class="flex justify-between text-xs text-gray-500 mt-1">
                                <span>0%</span>
                                <span id="opacity-value">{{ old('background_overlay_opacity', $pageHero->background_overlay_opacity) }}%</span>
                                <span>100%</span>
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="text_color" id="text_color" 
                                ...
                                <option value="white" {{ old('text_color', $pageHero->text_color) == 'white' ? 'selected' : '' }}>Putih</option>
                                <option value="dark" {{ old('text_color', $pageHero->text_color) == 'dark' ? 'selected' : '' }}>Gelap</option>
                            </select>
                            @error('text_color')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="text_alignment" id="text_alignment" 
                                ...
                                <option value="left" {{ old('text_alignment', $pageHero->text_alignment) == 'left' ? 'selected' : '' }}>Kiri</option>
                                <option value="center" {{ old('text_alignment', $pageHero->text_alignment) == 'center' ? 'selected' : '' }}>Tengah</option>
                                <option value="right" {{ old('text_alignment', $pageHero->text_alignment) == 'right' ? 'selected' : '' }}>Kanan</option>
                            </select>
            ...
            <div class="mt-6 bg-white shadow-sm border border-gray-200 rounded-lg">
                <div class="px-6 py-4 border-b border-gray-200">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Edit tombol aksi (opsional)</p>
                </div>
                <div class="px-6 py-4 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="cta_primary_text" id="cta_primary_text" 
                                   value="{{ old('cta_primary_text', $pageHero->cta_primary_text) }}"
                        ...
                        <div>
                            ...
                            <input type="url" name="cta_primary_url" id="cta_primary_url" 
                                   value="{{ old('cta_primary_url', $pageHero->cta_primary_url) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="cta_secondary_text" id="cta_secondary_text" 
                                   value="{{ old('cta_secondary_text', $pageHero->cta_secondary_text) }}"
                        ...
                        <div>
                            ...
                            <input type="url" name="cta_secondary_url" id="cta_secondary_url" 
                                   value="{{ old('cta_secondary_url', $pageHero->cta_secondary_url) }}"
        ...
        <div class="lg:col-span-1">
            ...
            <div class="bg-white shadow-sm border border-gray-200 rounded-lg mb-6">
                ...
                <div class="px-6 py-4">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $pageHero->is_active) ? 'checked' : '' }}
            ...
            <!-- New Image Preview -->
            <div id="image-preview-container" class="hidden bg-white shadow-sm border border-gray-200 rounded-lg mb-6">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">Preview Gambar Baru</h3>
                </div>
                <div class="p-6">
            ...
            <div class="bg-white shadow-sm border border-gray-200 rounded-lg">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">Preview Live</h3>
                </div>
                <div class="p-6">
                    <div id="hero-preview" class="relative h-32 bg-gray-100 rounded-lg overflow-hidden"
                         style="background-image: url('{{ $pageHero->background_image ? Storage::url($pageHero->background_image) : '' }}'); background-size: cover; background-position: center;">
                        <div id="preview-overlay" class="absolute inset-0" 
                             style="background-color: {{ $pageHero->background_overlay_color }}; opacity: {{ $pageHero->background_overlay_opacity / 100 }};"></div>
                        <div id="preview-content" class="absolute inset-0 flex items-center {{ $pageHero->text_alignment === 'left' ? 'justify-start pl-6' : ($pageHero->text_alignment === 'right' ? 'justify-end pr-6' : 'justify-center') }}">
                            <div id="preview-text" class="text-{{ $pageHero->text_alignment }} px-3" style="color: {{ $pageHero->text_color === 'dark' ? '#1f2937' : '#ffffff' }};">
                                <h4 id="preview-title" class="text-lg font-semibold mb-1">{{ $pageHero->title }}</h4>
                                <p id="preview-subtitle" class="text-sm opacity-90 {{ $pageHero->subtitle ? '' : 'hidden' }}">{{ $pageHero->subtitle }}</p>
                                <div id="preview-ctas" class="mt-2 space-x-2">
                                    @if($pageHero->cta_primary_text)
                                        <span class="inline-block px-3 py-1 bg-blue-600 text-white text-xs rounded">{{ $pageHero->cta_primary_text }}</span>
                                    @endif
                                    @if($pageHero->cta_secondary_text)
                                        <span class="inline-block px-3 py-1 bg-gray-600 text-white text-xs rounded">{{ $pageHero->cta_secondary_text }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    textAlignmentInput.addEventListener('change', function() {
        ...
        previewContent.className = `absolute inset-0 flex items-center ${
            alignment === 'left' ? 'justify-start pl-6' : 
            alignment === 'right' ? 'justify-end pr-6' : 
            'justify-center'
        }`;
        previewText.className = `text-${alignment} px-3`;
        previewText.style.color = textColorInput.value === 'dark' ? '#1f2937' : '#ffffff';
    });
