    ...
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@600&display=swap" rel="stylesheet">
        ...
        <section>
            <div class="container mx-auto px-4 py-10">
                {{-- Navigasi Tab --}}
                <div class="flex justify-center border-b border-red-800 text-lg font-semibold"
                    style="font-family: 'Poppins', sans-serif;">
                    <button onclick="showTab('batu')" id="tab-batu" class="cursor-pointer text-red-800 px-6 py-3">
                        (Kantor Pusat) Batu
                    </button>
                    <button onclick="showTab('karangploso')" id="tab-karangploso"
                        class="cursor-pointer text-red-800 px-6 py-3">
                        (Kantor Cabang) Karang Ploso
                    </button>
                    <button onclick="showTab('kas')" id="tab-kas" class="cursor-pointer text-red-800 px-6 py-3">
                        (Kantor Kas) UMM
                    </button>
                </div>

                {{-- Konten Tab --}}
                <div id="konten-batu" class="tab-content block" style="font-family: 'Poppins', sans-serif;">
                    @include('kontak-lokasi.batu')
                </div>
                <div id="konten-karangploso" class="tab-content hidden" style="font-family: 'Poppins', sans-serif;">
                    @include('kontak-lokasi.karangploso')
                </div>
                <div id="konten-kas" class="tab-content hidden" style="font-family: 'Poppins', sans-serif;">
            ...
            <script>
                function showTab(id) {
                    ...
                    tabs.forEach(tab => {
                        ...
                        document.getElementById('tab-' + tab).classList.remove('border-b-2', 'border-yellow-700', 'text-yellow-700');
                    });

                    document.getElementById('konten-' + id).classList.remove('hidden');
                    document.getElementById('tab-' + id).classList.add('border-b-2', 'border-yellow-700', 'text-yellow-700');
                }

                // Tampilkan tab default saat halaman pertama kali dibuka
